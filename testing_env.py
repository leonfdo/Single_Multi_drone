...
def map_grid(n):
    file_name="E:\Billy\Test_maps\obs"+str(n)+".csv"
...
class drone_env(gym.Env):
    def __init__(self,render_mode,num=0):
        self.reset_num=0
        self.random_num=num
        self.num=0
    ...
    def step(self, action):
        ...
        if colision:
            ...
            if cells[self.drone_pos[0][0]][self.drone_pos[0][1]]==0:
                ...
                reward_b = self.signal_strength*50
        ...
        if distance>self.RADIUS_TARGET_HIGH_SIGNAL and self.inside:
            reward_t=-10
        '''
        #print(self.reached(self.target_pos, self.drone_pos[0]))
        #print(distance)
        if distance <= self.RADIUS_TARGET_MEDIUM_SIGNAL:
            if(distance <= (self.RADIUS_TARGET_HIGH_SIGNAL+5) or (not self.reached(self.target_pos, self.drone_pos[0]))):
                self.done=True
                self.finished=True
        ...
        return self.observation, self.reward, self.done, self.info,self.finished,self.steps_taken
